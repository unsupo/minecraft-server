...
latest () {
	...
	curl https://api.papermc.io/v2/projects/paper/versions/$latest_version/builds/$latest_build/downloads/$latest_jar -o server.jar
}
[ -e server.jar ] && echo "server.jar already exists skipping download...." || latest
...
start () {
	# Starting the server
        ...
        echo "java -jar -Xms${ALLOCATED_RAM} -Xmx${ALLOCATED_RAM} ${JVM_ARGUMENTS} ${JVM_ARGUMENTS_RAM} server.jar nogui" > $script && chmod +x $script
